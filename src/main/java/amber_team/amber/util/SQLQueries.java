...
public class SQLQueries {
    ...
    public static final String GET_COMMENTS_OF_REQUEST = "SELECT id, user_id, comment_text, creation_date FROM comments WHERE request_id = ?";
    public static final String GET_USER_BY_ID = "SELECT * FROM users WHERE id = ?";
    public static final String REQUEST_ATTRIBUTES_VALUES_BY_ID = "SELECT attr_id AS id, A1.name AS name, A1.attr_order, A1.type AS type, string_value, date_value, decimal_value " +
            "FROM request_values AS RV1 INNER JOIN (SELECT A2.id AS id, A2.name AS name, AT1.name AS type, A2.attr_order AS attr_order FROM attributes AS A2 INNER JOIN attribute_types AS AT1 ON A2.attr_type_id=AT1.id) AS A1 ON RV1.attr_id=A1.id " +
