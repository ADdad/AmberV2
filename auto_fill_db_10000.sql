...
INSERT INTO users (id, email, password, s_name, f_name, enabled) SELECT * FROM new_users
ON CONFLICT DO NOTHING								
;
-- -------------------------------------------------------------------------------------------------------

INSERT INTO user_roles (role_id, user_id) SELECT 1, id FROM users LIMIT 50
ON CONFLICT DO NOTHING
;
-- -------------------------------------------------------------------------------------------------------


INSERT INTO user_roles (role_id, user_id) SELECT 2, id FROM users LIMIT 9850 OFFSET 50
ON CONFLICT DO NOTHING
;
-- -------------------------------------------------------------------------------------------------------


INSERT INTO user_roles (role_id, user_id) SELECT 3, id FROM users LIMIT 100 OFFSET 9900
ON CONFLICT DO NOTHING
;
-- -------------------------------------------------------------------------------------------------------
...
INSERT INTO equipment (id, model, producer, country) SELECT * FROM new_equip
ON CONFLICT DO NOTHING
;
-- -------------------------------------------------------------------------------------------------------
...
INSERT INTO warehouses (id, adress,  contact_number) SELECT * FROM new_warehouses
ON CONFLICT DO NOTHING
;
-- -------------------------------------------------------------------------------------------------------
...
INSERT INTO user_warehouses (user_id, warehouse_id) 
	SELECT u.id, w.id 
	FROM users AS u INNER JOIN warehouses AS w ON u.id = w.id
ON CONFLICT DO NOTHING
;
-- -------------------------------------------------------------------------------------------------------
...
WITH u_w AS (
	...
	FROM users AS u INNER JOIN warehouses AS w ON u.id = w.id || '0'
), u_3 AS (
	SELECT user_id
	FROM user_roles
	WHERE role_id = 3
	...
	FROM u_3 INNER JOIN u_w ON u_3.user_id = u_w.uid
), random_strings AS (
	SELECT generate_series(1,10000) AS id, 'str' || generate_series(1,10000) AS value
), random_numb AS (
	SELECT floor(random() * 10000 + 1)::int AS a
...
(SELECT r.value, w.id, u.id, 'str99' AS eid, 'iruds213' AS req_type_id, 'title' AS title, 'Opened' AS status, 'description' AS d, false AS arc
FROM ((random_strings AS r INNER JOIN warehouses AS w ON r.value = w.id)
	...
	LIMIT 4000)
	UNION 
	(SELECT r.value, w.id, u.id, 'str98' AS eid, 'iruds213' AS req_type_id, 'title' AS title, 'Opened' AS status, 'description' AS d, false AS arc
	FROM ((random_strings AS r INNER JOIN warehouses AS w ON r.value = w.id)
		INNER JOIN users AS u ON r.value = u.id)
		LEFT JOIN executors AS e ON r.value = e.uid
		LIMIT 3000 OFFSET 4000)
		UNION 
		(SELECT r.value, w.id, u.id, 'str97' AS eid, 'iruds213' AS req_type_id, 'title' AS title, 'Opened' AS status, 'description' AS d, false AS arc
			FROM ((random_strings AS r INNER JOIN warehouses AS w ON r.value = w.id)
			INNER JOIN users AS u ON r.value = u.id)
			LEFT JOIN executors AS e ON r.value = e.uid
